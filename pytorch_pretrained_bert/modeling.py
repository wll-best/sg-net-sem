...
###新增结束------>
    def forward(self, hidden_states, attention_mask, span_mask=None):
        ...
        if span_mask is not None:#if里的是新增的
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, span_mask=None):#span_mask是新增的
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True, span_mask=None):
        ...
        if span_mask is not None:#span_mask新增
...
class BertForTokenClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        if labels is not None:
            ...
            if attention_mask is not None:#新增的
...
class BertForQuestionAnsweringSpanMask(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_span_mask=None,
                start_positions=None, end_positions=None, is_impossibles=None):#新增is_impossibles
...
class BertForSequenceClassificationSpanMask(BertPreTrainedModel):
    ...
    """
    def __init__(self, config, num_labels):
        super(BertForSequenceClassificationSpanMask, self).__init__(config)
        self.num_labels = num_labels##
        self.bert = BertModel(config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, input_span_mask=None):
        ...
        ###开始变化。这四行squad里没有
        input_span_mask = input_span_mask.view(-1, input_ids.size(-1), input_ids.size(-1))###需要view吗？？？
        ...
        # span_attention_mask。这段用了两次，而squad里只用了一次
        extended_span_attention_mask = input_span_mask.unsqueeze(1)
        extended_span_attention_mask = extended_span_attention_mask.to(
            dtype=next(self.parameters()).dtype)  # fp16 compatibility
        extended_span_attention_mask = (1.0 - extended_span_attention_mask) * -10000.0
        ...
        pooled_output = self.pooler(sequence_output)#这一行squad里没有
        ###结束变化
...
class BertForMultipleChoiceSpanMask2(BertPreTrainedModel):
    ...
    def __init__(self, config, num_choices):
        ...
        self.apply(self.init_bert_weights)
        self.ddd=nn.Linear(config.hidden_size*2,config.hidden_size)###
    #新增input_span_mask
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, input_span_mask=None):
        input_span_mask = input_span_mask.cuda()###
        flat_input_ids = input_ids.view(-1, input_ids.size(-1))
        ...
        w = F.softmax(self.w)
        #sequence_output = self.gamma * (w[0] * sequence_output + w[1] * span_sequence_output)
        sequence_output=torch.cat([sequence_output,span_sequence_output],2)#改！！！！！
